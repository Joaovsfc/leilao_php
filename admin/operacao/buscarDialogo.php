<?php
    ...
    if($_POST){
        ...
        //$sql = "SELECT m.*, u.nome FROM mensagem  m JOIN usuario u ON m.id_remetente = u.id_usuario WHERE m.id_destinatario = :id_destinatario AND m.id_remetente = :id_remetente ORDER BY m.dt_mensagem ASC;";
        $sql = "SELECT m.*, u.nome FROM mensagem m JOIN usuario u on m.id_remetente = u.id_usuario where m.id_remetente = :id_remetente and m.id_destinatario = :id_destinatario ORDER BY m.dt_mensagem ASC";
